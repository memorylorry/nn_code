...
for epoch in range(1):
    ...
    for idx,batch_fn in enumerate(voc_dataset):
        ...
        output = model(batch_img, targets)
        loss = (output['loss_classifier'] + output['loss_box_reg'] + output['loss_objectness'] + output['loss_rpn_box_reg'])/4
        if idx % 100 == 99:
            batch_num += 1
            print("batch - %d" % batch_num,running_loss/100)
            running_loss = 0
        else:
...
torch.save(model.state_dict(),'sd_fast_r_cnn.pkl')